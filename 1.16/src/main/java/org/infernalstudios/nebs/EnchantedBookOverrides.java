...
public final class EnchantedBookOverrides extends ItemOverrideList {
     ...
     * {@link ItemOverrideList#ItemOverrideList(ModelBakery, IUnbakedModel, Function, Function, List)}. Here, all of the
     * registered enchantments are grabbed from the {@link ForgeRegistries#ENCHANTMENTS Enchantments registry} and are
     * queried for automatic model loading. The process of taking advantage of automatic model loading was described in
     * the documentation for the class in {@link EnchantedBookOverrides}.
     * <p>
     ...
     */
    private static BakeResult bakeOverrides(ModelBakery bakery, Function<ResourceLocation, IUnbakedModel> modelGetter, Function<RenderMaterial, TextureAtlasSprite> spriteGetter, Iterable<Enchantment> enchantments, int expected) {
        ...
        enchantments.forEach(enchantment -> {
            ...
            IBakedModel baked = bakery.getBakedModel(model, ModelRotation.X0_Y0, spriteGetter);
            if (baked == null) {
                missing.add(enchantment);
                return;
            }
